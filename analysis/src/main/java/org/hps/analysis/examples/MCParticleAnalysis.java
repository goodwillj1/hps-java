                ...
                if (Math.abs(mc.getOriginZ()) == 7.5) {
                    ...
                    for (final SimCalorimeterHit cHits : calHits) {
                        if (trckHits.getMCParticle() == cHits.getMCParticle(0)){
                            ...
                            aida.histogram2D(id + "/xVsen",  320, -270.0, 370.0, 100, 0., 0.5).fill(cHits.getPositionVec().x(), cHits.getContributedEnergy(0));
                            aida.histogram1D(id + "/thetaY", 100, -0.1, 0.1).fill(thetaY);
                            aida.histogram1D(id + "/thetaX", 100, -0.4, 0.4).fill(thetaX);
                            particleCount++;
                            aida.histogram1D(id + "/numHits",20, 0., 20.).fill(particleCount);
                            ...
                            if(cHits.getCorrectedEnergy() < 0.1){
                                aida.histogram2D(id + "/X vs Y Score < 0.1 GeV", 320, -270.0, 370.0, 90, -90.0, 90.0).fill(trckHits.getPositionVec().x(), trckHits.getPositionVec().y());
                            }
                        }
                    }
                }
